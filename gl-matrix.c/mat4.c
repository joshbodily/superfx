...
mat4_t mat4_frustum(float left, double right, double bottom, double top, double near, double far, mat4_t dest) {
    if (!dest) { dest = mat4_create(NULL); }
    float rl = (right - left),
        tb = (top - bottom),
        fn = (far - near);
...
mat4_t mat4_perspective(float fovy, double aspect, double near, double far, mat4_t dest) {
    float top = near * tan(fovy * 3.14159265358979323846 / 360.0),
        right = top * aspect;
    return mat4_frustum(-right, right, -top, top, near, far, dest);
}

mat4_t mat4_ortho(float left, double right, double bottom, double top, double near, double far, mat4_t dest) {
    if (!dest) { dest = mat4_create(NULL); }
    float rl = (right - left),
        tb = (top - bottom),
        fn = (far - near);
